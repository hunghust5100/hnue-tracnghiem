/* ========================================================
   100% PURE JAVASCRIPT DECRYPTION ENGINE (SAFE FOR HTTP 0.0.0.0, LOCALHOST & FILE://)
======================================================== */
const SECRET_PASSPHRASE = "HNUE_TRACNGHIEM_SECURE_KEY_2026";

function sha256_pure(array) {
    var chksum = [
...
async function decryptQuizArrayBuffer(arrayBuffer) {
    ...
    try {
        ...
        if (bytes.length < 13 || bytes[0] !== 0x48) {
        ...
        while (counter * 32 < ciphertext.length) {
            ...
            const ks = sha256_pure(blockInput);
        ...
        console.error("Pure JS Decrypt error:", e);
        try {
            return new TextDecoder('utf-8').decode(arrayBuffer);
        } catch(err) {
            return null;
