...
function encryptPure(text) {
    const enc = new TextEncoder();
    const plainBytes = enc.encode(text);
    const keyBytes = enc.encode(SECRET_PASSPHRASE);
    const iv = new Uint8Array(12);
    for (let i = 0; i < 12; i++) iv[i] = Math.floor(Math.random() * 256);

    const out = new Uint8Array(1 + 12 + plainBytes.length);
    out[0] = 0x48; // Magic 'H'
    out.set(iv, 1);

    let counter = 0;
    while (counter * 32 < plainBytes.length) {
        const blockInput = new Uint8Array(keyBytes.length + 12 + 4);
        blockInput.set(keyBytes, 0);
        blockInput.set(iv, keyBytes.length);
        blockInput[keyBytes.length + 12] = (counter >>> 24) & 0xff;
        blockInput[keyBytes.length + 13] = (counter >>> 16) & 0xff;
        blockInput[keyBytes.length + 14] = (counter >>> 8) & 0xff;
        blockInput[keyBytes.length + 15] = counter & 0xff;
...
function decryptPure(bytes) {
    ...
    let counter = 0;
    while (counter * 32 < ciphertext.length) {
        const blockInput = new Uint8Array(keyBytes.length + 12 + 4);
        blockInput.set(keyBytes, 0);
        blockInput.set(iv, keyBytes.length);
        blockInput[keyBytes.length + 12] = (counter >>> 24) & 0xff;
        ...
        const ks = sha256(blockInput);
        const offset = counter * 32;
        const count = Math.min(32, ciphertext.length - offset);
        for (let i = 0; i < count; i++) {
            out[offset + i] = ciphertext[offset + i] ^ ks[i];
        }
        counter++;
    }
...
const originalText = "Câu 1: Đối tượng nghiên cứu của Giáo dục học là gì?\n*A. Quá trình giáo dục con người.\nB. Các hiện tượng tự nhiên.";
console.log("Original Text:", originalText);

const encrypted = encryptPure(originalText);
console.log("Encrypted byte length:", encrypted.length);

const decrypted = decryptPure(encrypted);
console.log("Decrypted Text:", decrypted);

if (decrypted === originalText) {
    console.log("SUCCESS! Pure JS encryption/decryption matches 100%!");
} else {
